...
d,createtime,qdname,username,pnumber,wechat,project,jdy,login,description,gzjd,limits,czy 
from userdata 
...
d,createtime,qdname,username,pnumber,wechat,project,jdy,login,description,gzjd,limits,czy 
from userdata 
where `createtime` >= '$start'  AND `createtime` <= '$end'  AND limits = '$ulimits' ";}
		$result = mysqli_query($conn,$sql);
$data = array();
$i = 0;
while ($row = mysqli_fetch_array($result)){
    $ssqdname=$row['qdname'];
    $ssqdname=preg_replace('|[0-9a-zA-Z/<>=.:"_]+|','',$ssqdname); 
	$data[$i]['id'] = $row['id'];
...
/*$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1','提交日期');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B1','扫粉地段');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C1','姓名');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('D1','手机号');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('E1','年龄段');

$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F1','职业');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('G1','兴趣');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('H1','消费力');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I1','咨询项目');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('J1','预约时间');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('K1','入单');
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('L1','上门');
...
foreach($data as $key => $value){
	...
	/*
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$key,$value['createtime']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$key,$value['address']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$key,$value['username']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D'.$key,$value['pnumber']);
      */
...
//导出代码
$objPHPExcel->getActiveSheet() -> setTitle('三方统计数据列表');
$objPHPExcel-> setActiveSheetIndex(0);
